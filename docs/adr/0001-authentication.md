...
- Use 256-bit opaque refresh tokens. Store only an HMAC-SHA-256 digest, rotate on every use, and track a session family. Reuse of a rotated token revokes the entire family.
...
- Protect cookie-authenticated state changes with an independently generated CSRF token and constant-time comparison.
